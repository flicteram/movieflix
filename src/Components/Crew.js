...
function Crew(){
    ...
    useEffect(()=>{
        ...
        .then(data=>setCrew(data.crew.slice(0,10).map(item=>(
            <div className='crew'>
                {item.profile_path===null?<img className='user' src={user}/>:<img className='crewImg' src={`https://image.tmdb.org/t/p/w500/${item.profile_path}`}/>}
