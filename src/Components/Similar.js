...
function Similar(){
    ...
    useEffect(()=>{
        ...
        .then(data=>setSimilar(data.results.slice(0,8).map(item=><Link to={`movieflix/movie/details/${item.id}`}><img className='similarImg' src={`https://image.tmdb.org/t/p/w500//${item.poster_path}`}/></Link>)))
    },[movieId])
