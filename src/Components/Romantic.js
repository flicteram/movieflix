...
function Romantic(){
    ...
    let romanticDisplay = romantic.map(item=><Link key={item.id} to={`/movie/details/${item.id}`}><img  alt={item.id} className='romanticImg' src={`https://image.tmdb.org/t/p/w200/${item.poster_path}`}/></Link>)
