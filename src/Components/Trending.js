...
function Trending(){
    ...
    let trendingDisplay = poster.map(item=><Link to={`movieflix/movie/details/${item.id}`}><img key={item.id} alt={item.id} className='trendingImg' src={`https://image.tmdb.org/t/p/w200/${item.poster_path}`}/></Link>)
