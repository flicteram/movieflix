...
function TopRated(){
    ...
    let topRatedDisplay = topRated.map(item=><Link to={`movieflix/movie/details/${item.id}`}><img key={item.id} className='topRatedImg' alt={item.id} src={`https://image.tmdb.org/t/p/w200/${item.poster_path}`}/></Link>)
    return (
